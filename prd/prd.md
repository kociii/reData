...
系统根据项目字段定义动态生成 AI Prompt，包含字段名称、类型、必填标记和提取提示。AI 模型根据 Prompt 从原始数据中提取对应字段，并以 JSON 格式返回结果。

**详细的 Prompt 模板和生成逻辑请参考**: dev.md 第 6 章
...
### 4.1 表头识别规则
系统读取每个 Sheet 的前 5 行数据,提交给 AI 模型自动判断表头位置(第 1-5 行或无表头)。有表头时建立列索引与字段名的映射关系,无表头时标记为无表头模式直接处理原始数据。

### 4.2 数据提取规则
**有表头的表格**: 从表头行下一行开始,按照表头映射关系组装「表头:值」格式,提交给 AI 提取项目定义的字段。

**无表头的表格**: 从第 1 行开始,直接将行内容提交给 AI,从原始内容中提取项目定义的字段。

### 4.3 空行处理规则
维护空行计数器,遇到空行时 +1,遇到非空行时清零。当计数器达到 10 时,跳过当前 Sheet 的剩余内容。

### 4.4 多 Sheet 处理规则
遍历文件中的所有 Sheet,每个 Sheet 独立进行表头识别和数据提取,记录数据来源的 Sheet 名称。
